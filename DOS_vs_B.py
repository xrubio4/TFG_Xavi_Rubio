...
def VmagnAX(a,B):
    ...
    for i in range(N):
        for j in range(N):
            if i == j:
                V[i, j] = ((q*B*Z[i]/2)**2)/(2 * rmass*m0)  #Diagonal
            elif (i - j) == 1:
                V[i, j] = complex(0,-hbarr*q*B*Z[i]/(2*a* 2 * rmass*m0)) #Diagonal superior
            elif (i - j) == -1:
                V[i, j] = complex(0,hbarr*q*B*Z[i]/(2*a* 2 * rmass*m0)) #Diagonal inferior
    return V

# Crear una función para calcular la densidad de estados
def calcular_densidad_de_estados(energias, ancho_de_bandas=0.01):
...
#Tensor d'energia potencial n^3 x n^3
    U=np.kron(np.kron(VmagnAX(dx,i),I),I) + np.kron(np.kron(I,I),VmagnAZ(dz,i))
